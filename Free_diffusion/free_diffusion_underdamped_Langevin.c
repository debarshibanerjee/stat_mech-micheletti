...
int main() {
	...
	for (repetitions = 0; repetitions < n_samples; repetitions++) {
		...
		for (i = 0; i <= 100000; i++) { /* advance time step */
			...
			new_velocity = velocity - (gamma / mass) * velocity * dt +
						   sqrt(2 * KbT * gamma) / mass * eta * sqrt(dt);
		...
		mean_square_position +=
			position * position; /* cumulate the final square position to take the average */
	}
	fclose(fp);
	fclose(fp2);
	...
	printf("\n\nMean square position at time %lf is: %lf .  Expected:  %lf\n\n", time,
		   mean_square_position, 2.0 * KbT / gamma * time);
	return 0;
}
